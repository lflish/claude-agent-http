...
| 问题 | 影响范围 | 解决方案 |
|------|----------|----------|
| **Bun 在无 AVX 指令集 CPU 上崩溃** | KVM 虚拟机、不支持 AVX 的旧 CPU | v1.1.0 已修复：切换到 Node.js 运行时（`npm install -g @anthropic-ai/claude-code`） |
| **多会话 OOM** | 内存有限的主机 | 使用 `mem_limit` + `memory_limit_mb`（参见内存保护） |

> **说明**：自 v1.1.0 起，Claude Code CLI 运行在 Node.js 上，不再使用 SDK 内置的 Bun 二进制文件，确保兼容所有 x86_64 CPU（无需 AVX 指令集支持）。
...
- 基于 [Claude Agent SDK](https://github.com/anthropics/claude-agent-sdk)（Python API）+ [Claude Code CLI](https://www.npmjs.com/package/@anthropic-ai/claude-code)（Node.js 运行时）构建
